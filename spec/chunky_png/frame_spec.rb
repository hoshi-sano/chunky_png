...
describe ChunkyPNG::Frame do
  describe '.from_canvas' do
    ...
    it 'should read a stream without failing' do
      ...
      expect(frame.width).to eq(1)
      expect(frame.height).to eq(1)
      expect(frame.delay_num).to eq(1)
      expect(frame.delay_den).to eq(10)
    end
  end
  ...
  describe '.from_chunks' do
    ...
    context 'initial frame (default image)' do
      ...
      it 'should read a stream without failing' do
        ...
        expect(subject.delay_num).to eq(1)
        expect(subject.delay_den).to eq(30)
  ...
  describe '#to_frame_control_chunk' do
    ...
    it do
      ...
      expect(subject.sequence_number).to eq(5)
      expect(subject.width).to eq(1)
